...
def test_audit_flags_known_tics_in_order():
    t = (
        "Listen to how the bass enters. Here's the thing: it isn't just a chord. "
    ...
    assert (
        "negation-just" in names
        and "reduction" in names
        and "machinery-naming" in names
    )
...
def test_clean_commentary_scores_zero():
    clean = (
        "Jefferson opens by quoting the Declaration he wrote. The music bends "
...
def test_platitude_rate_per_1000_words():
    ...
    assert (
        platitude_rate("here's the deal then some more filler words right now") == 100.0
    )
