...
Render a [`Script`](braidio.script.md#braidio.script.Script) into an audio file.

Walks the beats (filtered by the rights [`Profile`](braidio.rights.md#braidio.rights.Profile)):
narration beats are synthesized ([`braidio.tts`](braidio.tts.md#module-braidio.tts)) — as one call, or as paced
turns when the [`WeaveConfig`](braidio.weave_config.md#braidio.weave_config.WeaveConfig) asks for a smaller
`segmentation_unit` ([`braidio.pacing`](braidio.pacing.md#module-braidio.pacing)); segment beats are resolved
via a [`SegmentSource`](braidio.sources.md#braidio.sources.SegmentSource) and extracted (padded + faded);
scene breaks become a sting or a pause ([`braidio.structure`](braidio.structure.md#module-braidio.structure)). Every spoken
or clipped part is loudness-normalized, then either woven on a timeline (clips
tuck under narration — [`braidio.weave.weave_timeline()`](braidio.weave.md#braidio.weave.weave_timeline)) when a
[`WeaveConfig`](braidio.weave_config.md#braidio.weave_config.WeaveConfig) enables it, or concatenated.
